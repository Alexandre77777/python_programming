...
adres_request = urllib.request.urlopen("https://www.summet.com/dmsi/html/codesamples/addresses.html").read().decode()
...
#
# То есть, общий смысл данного выражения заключается в поиске информации в HTML-коде, где предполагается, что данные о именах, улицах, штатах и номерах хранятся в виде последовательности, отделенной от остального HTML-кода тегами "<li>".
